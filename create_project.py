...
def dumpUsage():
    ...
    print "  -language  PROGRAMING_LANGUAGE   Major programing lanauge you want to used, should be [cpp | lua | javascript | cqwrap]"
    print ""
...
def checkParams(context):
    ...
    if ("cpp" == context["language"]):
        ...
        platforms_list = ["ios",
                          "android",
                          "win32"]
    elif ("cqwrap" == context["language"]):
        context["src_project_name"] = "HelloJavascript"
        context["src_package_name"] = "org.cocos2dx.hellojavascript"
        context["src_project_path"] = os.getcwd() + "/../../template/multi-platform-cqwrap"
        platforms_list = ["ios",
                          "android",
                          "web"]
...
def processPlatformProjects(platform):
    ...
    if (platform == "android"):
        ...
        for i in range(0, len(dst_pkg)):
            if(i < 3):
                os.rename(tmp + src_pkg[i], tmp + dst_pkg[i])
            else:
                os.mkdir(tmp + dst_pkg[i])
                shutil.move(tmp + context["src_project_name"] + '.java', tmp + dst_pkg[i])
            tmp = tmp + dst_pkg[i] + "/"

        java_package_path = "/".join(dst_pkg)
